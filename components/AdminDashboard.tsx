  ...
  <div>
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {seller.phone && (
          ...
          href={`https://wa.me/${seller.phone.replace(/[\s\-()]/g, '').replace(/^\+/, '')}?text=${encodeURIComponent(
            `Hi ${seller.name}, you have a new order on Kano Market:\n\n` +
            `📦 Product: ${o.productTitle}\n` +
            `💰 Amount: ₦${o.amount.toLocaleString()}\n` +
            `👤 Buyer: ${o.buyerName ?? '—'}\n` +
            (o.buyerPhone ? `📞 Buyer Phone: ${o.buyerPhone}\n` : ``) +
            (o.buyerEmail ? `📧 Buyer Email: ${o.buyerEmail}\n` : ``) +
            (o.buyerAddress ? `📍 Delivery Address: ${o.buyerAddress}\n` : ``) +
            `\nPlease fulfil this order promptly. Thank you!`
          )}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 bg-[#25D366] hover:bg-[#1ebe5d] text-white text-xs font-bold px-2.5 py-1 rounded-lg transition-colors"
        >
          <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
