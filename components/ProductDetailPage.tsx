// components/ProductDetailPage.tsx  — DIFF: add Buy Now button
// Find the contact buttons section and add this button ABOVE the "Message on App" button.
// Also add the PaymentModal import and state at the top of the component.

// ── 1. Add these imports at the top of ProductDetailPage.tsx ─────────────────
// import { PaymentModal } from './PaymentModal';

// ── 2. Add this prop to ProductDetailPageProps ───────────────────────────────
// currentUser: User | null;
// onBuyNow?: (product: Product) => void;   ← optional override, or handle inline

// ── 3. Add this state inside the component ───────────────────────────────────
// const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);

// ── 4. Replace the contact buttons <div className="space-y-2.5"> with this ──
...
<div className="space-y-2.5">
  ...
  <button
    onClick={() => {
      if (!currentUser) {
        // trigger login — pass this up if needed
  ...
  {/* ... rest of your existing buttons (Message on App, WhatsApp, Call, Save) ... */}

</div>
*/

// ── 5. Add the PaymentModal at the bottom of the component return, before </div> ──
...
// ── FULL UPDATED FILE ─────────────────────────────────────────────────────────
// Below is the complete ProductDetailPage.tsx with all changes applied.
...
export const ProductDetailPage: React.FC<ProductDetailPageProps> = ({
  product, seller, currentUser, onClose, onMessageSeller, onLoginClick,
  isSaved, onToggleSave, onPaymentSuccess,
}) => {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 md:sticky md:top-24">
              ...
              <div className="space-y-2.5">
                ...
                {/* Call */}
                {hasPhone ? (
                  <a href={`tel:${dialPhone}`} className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors shadow-md shadow-blue-200 dark:shadow-blue-900/30 text-sm">
              ...
              {seller && (
                <div className="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="flex items-center gap-3">
                    <img src={seller.profilePicture} alt={seller.name} className="w-12 h-12 rounded-xl object-cover ring-2 ring-orange-50 dark:ring-orange-900/20 flex-shrink-0" />
      ...
      {isLightboxOpen && images.length > 0 && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4" onClick={() => setIsLightboxOpen(false)}>
          <button className="absolute top-4 right-4 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors" onClick={() => setIsLightboxOpen(false)}>
            <Icon name="close" className="w-5 h-5" />
          </button>
          <img src={images[selectedImageIndex]} alt={product.title} className="max-w-full max-h-[85vh] object-contain rounded-lg" onClick={e => e.stopPropagation()} />
          {images.length > 1 && (
            <>
              <button className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white" onClick={e => { e.stopPropagation(); setSelectedImageIndex(i => i === 0 ? images.length - 1 : i - 1); }}>
                <svg className="w-5 h-5 rotate-180" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
              </button>
              <button className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white" onClick={e => { e.stopPropagation(); setSelectedImageIndex(i => i === images.length - 1 ? 0 : i + 1); }}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
              </button>
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
                {images.map((_, idx) => (
                  <button key={idx} onClick={e => { e.stopPropagation(); setSelectedImageIndex(idx); }} className={`w-2 h-2 rounded-full transition-all ${idx === selectedImageIndex ? 'bg-white scale-125' : 'bg-white/40'}`} />
                ))}
              </div>
            </>
          )}
